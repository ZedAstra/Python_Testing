...
from flask import (Flask, render_template,
                   request, redirect, flash, url_for, Response)
import models


def create_cookie(club):
    time = datetime.now() + timedelta(days=1)
    user = models.User.create(token=uuid.uuid4(),
                              valid_until=time,
                              linked_club=club)
    return ("authentication="+str(user.token)+"; expires=" +
...
def loadClubs():
    with open('clubs.json') as clubs:
        ...
        for club in listOfClubs:
            ...
            if len(q) == 0:
                models.Club().create(name=club['name'],
                                     email=club['email'],
...
def loadCompetitions():
    with open('competitions.json') as comps:
        ...
        for competition in listOfCompetitions:
            q = models.Competition().select().where(
                models.Competition.name == competition['name'])
            if len(q) == 0:
                (models
                 .Competition()
...
def create_app():
    ...
    def index():
        if check_cookie(request):
            ...
            q = (models.User()
                 .select()
                 .where(models.User.token == uuid.UUID(token))
                 .limit(1))
            if len(q) == 0:
    ...
    def summary():
        ...
        if check_cookie(request):
            if request.form.get('email'):
                q = (models.Club()
                     .select()
                     .where(models.Club.email == request.form['email'])
                     .limit(1))
                if (len(q) == 0):
                    flash("No club found with this email")
                    return render_template('index.html')
                club = q[0]
                res = Response(render_template('welcome.html',
                                               club=club,
            ...
            if request.form.get("email"):
                clubs_trouves = [
                    club for club in models.Club.select()
                ...
                if clubs_trouves:
                    ...
                    q = (models.User()
                         .select()
                         .where(models.User.linked_club == club)
                         .limit(1))
                    if len(q) == 0:
                        res = Response(
                        ...
                        if q[0].valid_until < datetime.now():
                            ...
                            res.headers["Set-Cookie"] = ("authentication=" +
                                                         str(q[0].token))
                            return res
                else:
                    club = None
    ...
    def book(competition, club):
        ...
        q2 = (models.Competition()
              .select()
              .where(models.Competition.name == competition)
              .limit(1))
    ...
    def purchasePlaces():
        q1 = (models.Competition()
              .select()
              .where(models.Competition.name ==
                     request.form.get("competition")))
        q2 = (models.Club()
              .select()
              .where(models.Club.name == request.form.get("club")))
        if len(q1) == 1 and len(q2) == 1:
            competition: models.Competition = q1[0]
            club: models.Club = q2[0]
            ...
            if (club.points >= placesRequired):
                ...
                q3 = (models.Participation()
                      .select()
                      .where(models.Participation.competition == competition,
                             models.Participation.club == club))
                if len(q3) == 1:
                    participation: models.Participation = q3[0]
                    participation.contestants = (participation.contestants +
                                                 placesRequired)
                    participation.save()
                else:
                    models.Participation.create(club=club,
                                                competition=competition,
                                                contestants=placesRequired)
                flash('Successfully booked ' + str(placesRequired) +
                      'place(s)!')
    ...
    def logout():
        ...
        res.headers.set("Set-Cookie", "authentication=;" +
